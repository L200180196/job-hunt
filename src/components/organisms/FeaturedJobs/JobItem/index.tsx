...
const JobItem: FC<JobItemProps> = ({ image, jobType, name, type, location, desc, category, skills }) => {
    return (
      <div className='border border-border p-6 cursor-pointer'>
        ...
        <div className='space-x-2'>
          {skills.map((item: string, i: number) => (
              <Badge key={item+i} variant="outline" className="rounded border-primary bg-primary bg-primary/5 text-primary">{item}</Badge>
          ))}
        </div>
      </div>
    )
